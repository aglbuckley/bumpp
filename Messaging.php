                ...
                if(!$stmt->bind_param("ii", $conversations[0], $userID)){
                    ...
                    if(!$stmt->execute())
                    {
                        ...
                        if(!$stmt->bind_result($photoLocation))
                        {
                            ...
                            while($stmt->fetch())
                            {
                                $response = $response.'<a class="th" id = '.$conversations[$i].' data-options="align:right" data-dropdown="conversation-drop" onclick="fetchConversation('.$conversations[$i].')" href="#">
